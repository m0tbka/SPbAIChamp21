...
from strategy.flying_group_advanced import FlyingGroupAdv, AllFlyingGroups
...
class RoadNet:
    ...
    def __init__(self, vertices, my_index, max_distance, players):
    ...
    """Передаешь сейчас тик, домашнюю планету, твои планеты(и только твои), свойства зданий(game.building_properties),
     flying_groups"""

    def go_to_home(self, current_tick, home_planets, my_planets, building_properties, flying_groups: AllFlyingGroups):
        my_planets_id = [p.id for p in my_planets]
...
def find_planet(planets_got, harvestable_resource_to_find: Resource = None, free_resource_to_find=None,
    ...
    for planet in planets:
        if planet.harvestable_resource == harvestable_resource_to_find and clause_b(planet, building_to_find) and \
